...
$vPhone= "select contact from teacher where contact= '$phone' and id<>'$id'";
...
$vEmail= "select email from teacher where email= '$email' and id<>'$id'";
...
if((mysqli_num_rows($phone_valid)>0) || (mysqli_num_rows($phone_valid1)>0)){
    ...
    header("location: update_teacher.php?error='Phone'&id=$id");
}
else if((mysqli_num_rows($email_valid)>0) || (mysqli_num_rows($email_valid1)>0)){

    header("location: update_teacher.php?error='Email'&id=$id");
} 
else{
    $sql = "update teacher set fname='$fname', lname='$lname', contact='$phone', email='$email', courseName='$course', user_name='$uname', password='$pwd' where id='$id' ";
